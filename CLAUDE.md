...
- **Consult Sol (2nd-opinion model):** `codex` CLI is on PATH (config default model = `gpt-5.6-sol`, subagents `gpt-5.6-terra`). Non-interactive at max reasoning: `codex exec -c model_reasoning_effort="xhigh" -s read-only --skip-git-repo-check - < prompt.txt`; add `-m <model>` only to override the default. Pattern [USER]: spawn an Opus subagent to interface — it writes the prompt, runs codex, relays the answer. xhigh takes minutes → long Bash timeout; the startup stale-temp-dir warning is benign and unfixable (every run leaves one populated dir in `~/.codex/tmp/arg0`, and the next run's non-recursive rmdir trips on it).
