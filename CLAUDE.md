...
- **Compute — where to run + how to switch** (CS-iLab direct GPUs / iLab SLURM `ilab1` / Amarel HPC; auth + filesystem map + fallback order): the global **`compute-resources`** skill (`~/.claude/skills/compute-resources/` — a *user* skill, so it does NOT travel with this repo; install it on a fresh box). Key: CS-iLab = Kerberos (`ssh ilab1`, concrete host, shared FS ⇒ no copy); Amarel = SSH key + push/pull/rebuild.
