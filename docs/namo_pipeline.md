...
- Obstacles are inflated by a rotation-safe robot radius `R = sqrt(hx^2 + hy^2)` plus the tier-1 margin (default **0.005 m**), where `hx, hy` are robot half-extents in meters.
...
- Inflating obstacles by `sqrt(hx^2 + hy^2) + tier1_margin` (rotation-safe center-space inflation).
