...
## Failure mode #7: changing clearance configuration invalidates the evaluated population

The 2026-09-06 no-edge-ID handoff advanced its dedicated checkout past the 5 mm to 1 mm change in `config/wavefront_inflation.yaml`. The canonical manifests stayed at 1,328/992 episodes, but the evaluator skipped 59/25 rooms as already open and emitted only 1,247/966 rows. All workers exited successfully; strict aggregation exposed the mismatch. Pin the evaluation configuration together with code, bindings, and checkpoints when reusing registered controls. Do not lower expected counts to accept this as the same experiment. Also distinguish manifest room counts (997/958) from episode counts (1,328/992): `eval_bestfirst` slices room keys and then iterates every object/goal record in each selected room.

The simulator and ranker renderer must both receive the pinned configuration. On 2026-09-07 a policy smoke used 5 mm simulation but 1 mm masks because `BeamPlanner` omitted `LiveScorer.render_config`. Passing the config exposed another issue: Python's wavefront exporter resolved a primary-YAML symlink before locating the sidecar, while C++ used the caller-selected directory. Preserve that directory when loading the margin. Control-prefix parity caught the mismatch before a full evaluation was submitted; Random alone cannot detect a rendering mismatch because it does not use the renderer.
