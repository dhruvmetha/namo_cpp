...
Camden-hosted partitions are **`c`-prefixed**: `cgpu-redhat`, `cmain-redhat`, `cmem-redhat`, `cmain`. **Never submit to `cgpu-redhat`.** Every `gpu`/`gpu-redhat` node has the `piscataway` feature; Camden nodes don't — so staying on `gpu,gpu-redhat` already avoids Camden.
