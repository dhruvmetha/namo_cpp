...
| path | what | status |
|---|---|---|
| `/scratch/dm1487/manifests/test_pure2push_combined.txt` | 985 GENUINE depth-2 scenes (per-episode `_pair_` xmls); beam baseline 16%@1→56%@2 | **USING** (leaf diagnostic) |
| `/scratch/dm1487/manifests/test_pure2push_hardness.csv` | per-scene hits/depth2_hits + bucket (easy/med) | reference |
| `/scratch/dm1487/manifests/test_2push_solvable_combined.txt` | 1186 scenes, collisions-allowed-defined | **REJECTED for leaf diag** — 1-push-contaminated (smoke: every leaf rank-0) |
| `/scratch/dm1487/manifests/test_1push_solvable_combined.txt` | 539 scenes | not yet used |
...
| path | what |
|---|---|
| `/scratch/dm1487/sage_outputs/scorer/sharp_s1/namo-classifier/9yizg6i8/checkpoints/epoch017-val_loss0.2713.ckpt` | champion `sharp` 1-push scorer (the leaf) |
...
| env XMLs | `/scratch/dm1487/datasets/car_envs/v3/test/{aug9_car,feb_car}/…/env_*_pair_*.xml` |
...
| `/scratch/dm1487/eval/diag_leaf_s1_pure2push.json` | **H1/H2 result** (N=25 pure2push): H1 REJECTED (recall@10=.877), H2 ACCEPTED (AUC=.534) |
| `/scratch/dm1487/eval/diag_fpv_shard{0..7}.{json,jsonl}` | H3 per-leaf data, 240 pure2push scenes — PENDING (array `55815040`) |
| `/scratch/dm1487/eval/diag_fpv_aggregate.json` | H3a verdict — pending |
| `/scratch/dm1487/eval/diag_fpv_jobid.txt` | array job id (55815040) |
| `/scratch/dm1487/logs/diag_fpv-55815040_*.{out,err}` | array task logs |
| `/scratch/dm1487/eval/diag_leaf_s1_smoke.json`, `diag_fpv_smoke.{json,jsonl}` | smoke outputs (disposable) |
...
| file | finding |
|---|---|
| `/scratch/dm1487/eval/diag_leaf_s1_pure2push.json` | H1 REJECTED (leaf recall@10=.877), H2 ACCEPTED (V=maxP AUC=.534) |
| `/scratch/dm1487/eval/diag_fpv_shard{0..7}.jsonl` | 4486 per-leaf (s0,a1)→good/dead records + scalars (H3b seed data) |
| `/scratch/dm1487/eval/diag_fpv_aggregate.json` | H3a: best single scalar `mean_top5` AUC .73 (full) |
