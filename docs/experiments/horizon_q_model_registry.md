...
- **`BfixSL_s{1,2,3}` — NOT a separate condition, a cross-hardware reproduction of `Bfix`.** Same H5, same seeds, trained on rlab2 A100s instead of westeros 2080 Tis. val_loss 1.8619/1.8645/1.8687 vs westeros 1.8569/1.8629/1.8713 — agreement to ~0.005, which is the free check that nothing about the westeros setup was flaky. **Not evaluated; do not cite as an arm.** Also measured there: A100 epoch 181 s vs 2080 Ti ~1,200 s (6.6×) on this model.
- **`BL10_s{1,2,3}` — CANCELLED mid-training [USER 2026-08-08], no usable checkpoints.** Was Bfix at `LOWER_RANK_LAMBDA=0.10`; stopped at epoch 1–3 once the λ curve on arm A came back flat and the guess-exclusion fix superseded it. Re-running λ=0.10 on Bfix only makes sense *with* `NAMO_RANK_EXCLUDE_GUESS=1`, else it inherits the 593-tier pathology and costs 4× for an uninterpretable number.
- **`AJ2_s{1,2,3}` / `AJ2NR_s{1,2,3}` — arjuna-0 v2, COMPLETE 2026-08-08.** train_h5 `round0/arjuna0v2_train.h5` — **a third label condition, neither arm A nor arm B**: same base as both (`round3/h5/d20_plus_setup_only.h5`) but with **zero bootstrap guesses**; opener 1.0 / setup 0.5 / every bounded cell 0.0 exact / untried still masked. Zeros 0.66% → 48.2%. `AJ2` = `RANK_LAMBDA=LOWER_RANK_LAMBDA=0.1`; `AJ2NR` = both 0 (aux ablation). 12 epochs, 3 seeds. Aggregate `round0/gate_aj2.json` (spec `arms_aj2.json`), AUC panel `round0/auc_aj2.json`, eval dirs `round0/eval_bfix/AJ2{,NR}_s*/{1push_hmax2,2push}/` (432/432 shards, zero unmatched), Amarel originals `/cache/home/dm1487/aquaman0/eval_bfix/`. **`val_loss` is NOT comparable to any other row here** — 48% of its cells are zeros, so the loss scale moved; the canonical deploy gate is the only arbiter. Card: [EXP-2026-08-08-arjuna-hard-labels](log/EXP-2026-08-08-arjuna-hard-labels.md).
  - **Checkpoint provenance is NOT uniform** — seed 1 of `AJ2` comes from `models/AJ2_s1_a100/` (a duplicate run raced onto rlab2 because the ilab1 original was 4.5× slower; the original 203552 was cancelled at epoch ~9). Same H5, same seed, same epochs — the same run, faster hardware. Every other seed is from its plain `models/<NAME>_s<k>/` dir. **The other three racer dirs hold NO usable model and must never be cited:** `AJ2NR_s1_a100` was CANCELLED as the losing racer once the ilab1 original completed first (partial checkpoint, ~epoch 3), and `AJ2_s1_a6k` / `AJ2NR_s1_a6k` FAILED at 54 s on rlab7 and produced nothing. Safe to delete all three.
  - **Results, 3 seeds mean [min,max]:** `AJ2` 2p-hard@5 **26.8 [24.8, 28.5]**, @900 90.0, 1p-hard@1 38.1, s2s 105.0. `AJ2NR` 2p-hard@5 **19.9 [17.5, 21.9]**, @900 87.8, 1p-hard@1 29.4, s2s 134.5. **Floor hypothesis falsified at full dose** — `AJ2`'s band contains arm A (27.7), ARJ v1 (27.7) and Bfix (28.9); BNG (32.1) is above the whole band and stays the best model. **The keeper result is the ablation:** hard labels nearly double the no-aux model (`BfixNR` 11.2 → `AJ2NR` 19.9) and cut the aux's marginal value from +17.7 to +6.9 — labels and ranking are substitutes, and the earlier "it's all ranking" reading held only while the labels were degenerate.
- **`XB_s{1,2,3}` / `RP_s{1,2,3}` — cross-board ranking round 1, COMPLETE 2026-08-09.** Same train_h5 as AJ2 (`round0/arjuna0v2_train.h5`), only the LOSS differs. `XB` = AJ2 loss + batch-flat cross-board rank term (`XB_LAMBDA=0.1`, `train_q2_rankaux.py`); `RP` = rank-pure, regression + censored deleted, per-board + batch-flat terms at 1.0 each (`train_q2_rankpure.py`; **its `val_loss` is a RANK loss — comparable to nothing else in this file**). 12 epochs, 3 seeds, ckpts `round0/models/{XB,RP}_s{1,2,3}/checkpoints/`, Amarel copies `/cache/home/dm1487/aquaman0/ckpts_bfix/`. Canonical protocol identical to AJ2 row (1322 1push@hmax2 + 1012 2push, budget 900, combine=q, discount off, dedupe+jam on), 432/432 shards, zero unmatched. Aggregate `round0/gate_xbrp.json` (spec `arms_xbrp.json`), AUC panel `round0/auc_xbrp.json`, eval dirs `round0/eval_bfix/{XB,RP}_s*/`. **Results (3 seeds pooled):** XB 2p-hard@5 28.7 / @30 54.7 / @900 91.0 / s2s 99.6, 1p-hard@1 38.2; RP 2p-hard@5 24.3 / @2 12.9 / 1p-hard@5 80.9. Key facts: XB's V5 is FLAT vs AJ2 (0.456 vs 0.455 hard) — its deploy gain is finish-ordering, not cross-board repair; RP proves regression ≈ replaceable at deploy (vs the historic no-aux collapse). Card: [EXP-2026-08-09-crossboard-ranking](log/EXP-2026-08-09-crossboard-ranking.md).
- **`MM_s{1,2,3}` / `EG_s{1,2,3}` / `EGMM_s{1,2,3}` / `RPB_s{1,2,3}` — cross-board round 2, canonical COMPLETE 2026-08-09 night (all 12 seeds complete).** Same H5 + protocol as the XB/RP row; only the loss differs (all via `train_q2_round2.py` except RPB via `train_q2_rankpure.py` + `RP_BRAKE=0.02`). `MM` = +batch-flat margin-vs-max (`MM_LAMBDA=0.1`, margin 0.2); `EG` = +family softmax (`EG_LAMBDA=0.1`, grouped sampler); `EGMM` = +family margin-vs-max (`EGMM_LAMBDA=0.1`). Aggregate `round0/gate_r2.json` (spec `arms_r2.json`), panel `round0/auc_round2.json`, plots `round0/r2full_success_vs_sims.png`. **Results (2p-hard@5 pooled [min,max]):** MM **31.6 [27.7, 38.0]**, EG 29.7 [26.3, 31.4], EGMM 27.3 (3s final), RPB 23.4 — MM and EG are BNG-class (32.1) on honest categorical labels, no guesses. EGMM: only robust V5 mover (0.492 3-seed offline) + best F2 (0.920) + s2s-hard 93 at 3 seeds (2-seed 81 was subset-flattered) with dulled front-curve (V6 drop suspect). RPB falsifies the 2%-brake story. EGMM_s1 provenance: crashed twice on oversubscribed ilab GPUs (OOM), third submission clean on rlab. Card: [EXP-2026-08-09-crossboard-ranking](log/EXP-2026-08-09-crossboard-ranking.md).
- **`RPE_s{1,2,3}` — rank-pure EGMM (value-free stack), COMPLETE 2026-08-10.** `train_q2_rankpure_egmm.py` (RP's two softmax terms + family margin `RPE_FAM=1.0`, NO regression; **val_loss is a rank loss, comparable only to RP-family rows**), same H5/protocol as the XB row. Aggregate `round0/gate_rpe.json`, panel `round0/auc_rpe.json`, eval dirs `round0/eval_bfix/RPE_s*/`, 216/216 shards. Results: 2p-hard@5 25.9, @900 90.9 (2nd-best ledger reach), s2s 104; offline V6 0.720 (> parent EGMM 0.685), no histogram walls — both stacking tripwires clean, RPU unbounded arm not triggered. Card: [EXP-2026-08-09-crossboard-ranking](log/EXP-2026-08-09-crossboard-ranking.md).
- **`r1_dt_s{1,2,3}` — round-1 depth-token models, TRAINED BUT NEVER EVALUATED (2026-08-02).** ckpts `…/aquaman/round1/models/r1_dt_s{1,2,3}/checkpoints/epoch009-val_loss1.8324.ckpt` (54.8 MB — a *different architecture* from every other row here at 52.9 MB; local depth-token attention, sage `6f90dc6`). Their canonical eval was the 204-task Amarel sweep that died on a stale `sage` checkout without `action_motion_dim`; by the time the version was fixed the round-1 data itself had been rejected ("round-2/round-1 data was bad, not the screen's fault" [USER]), so the eval was never refired. **Zero deploy numbers exist for these — never cite them.** Kept only because the architecture arm is untested and the checkpoints are the cheap half of retesting it.
- **Smoke artifacts, never results:** `smokeB_s1`, `smokeW_Bfix`, `smokeNG` (1-epoch pipeline checks). Safe to delete.
- **Plot:** `round0/armB_success_vs_sims.png` — success vs simulator calls, arm-B family + random, 2×4 horizon × difficulty, 3 seeds pooled. Script `round0/plot_armB_curves.py` (tier functions copied from `aquaman_agg.py` so binning matches these tables).
