...
- **`MM_s{1,2,3}` / `EG_s{1,2,3}` / `EGMM_s{1,2,3}` / `RPB_s{1,2,3}` — cross-board round 2, canonical COMPLETE 2026-08-09 night (EGMM_s1 eval trailing; its 2-seed pool is the registered number until updated).** Same H5 + protocol as the XB/RP row; only the loss differs (all via `train_q2_round2.py` except RPB via `train_q2_rankpure.py` + `RP_BRAKE=0.02`). `MM` = +batch-flat margin-vs-max (`MM_LAMBDA=0.1`, margin 0.2); `EG` = +family softmax (`EG_LAMBDA=0.1`, grouped sampler); `EGMM` = +family margin-vs-max (`EGMM_LAMBDA=0.1`). Aggregate `round0/gate_r2.json` (spec `arms_r2.json`), panel `round0/auc_round2.json`, plots `round0/r2full_success_vs_sims.png`. **Results (2p-hard@5 pooled [min,max]):** MM **31.6 [27.7, 38.0]**, EG 29.7 [26.3, 31.4], EGMM(2s) 26.3, RPB 23.4 — MM and EG are BNG-class (32.1) on honest categorical labels, no guesses. EGMM: only robust V5 mover (0.492 3-seed offline) + best F2 (0.920) + best s2s-hard ever (81) with dulled front-curve (V6 drop suspect). RPB falsifies the 2%-brake story. EGMM_s1 provenance: crashed twice on oversubscribed ilab GPUs (OOM), third submission clean on rlab. Card: [EXP-2026-08-09-crossboard-ranking](log/EXP-2026-08-09-crossboard-ranking.md).
