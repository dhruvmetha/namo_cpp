...
**find_cc fix DEFERRED (documented, needs review — NOT done):** it's core region-labeling feeding the search; region-3+ IDs are seeded via `select_random_point` over that hash set, so a hash/DS change could reorder labels on scenes the (small-grid) gate doesn't cover. SAFE way to do it: (1) add a LARGE-grid scene to the golden REF; (2) make region-3+ seeding deterministic grid-scan order; (3) replace the `unordered_set` with the `region_grid_` 2D visited-marker (O(1), no hash); (4) gate on car-180 + the large-grid golden. Huge win (32s→<1s) but review-worthy. The active CAR pipeline is NOT hit by this (1.2ms).
