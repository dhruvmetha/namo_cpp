...
| 08-13 | **deep budget: 4000 simulator calls** (model ×3 vs random ×3, both re-run under identical settings) | does the model just solve MORE, or solve FASTER? | 📊 **efficiency, not capability** — at 4000 calls hard-tier solve rates converge (model 100.0 vs random 97.2, vs a 30-point gap at 900); the durable claim is **7.2× fewer calls on hard, 6.5× medium, 3.3× easy** | [crossboard](log/EXP-2026-08-09-crossboard-ranking.md) |
