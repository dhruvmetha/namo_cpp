...
**Bonus-episode ledger (flagged, verification in flight):** the round-1 collector (target_goal_region ON, room-wise per the data invariant) surfaced ~7.9-13.3k episodes beyond the antman file's curated keys — additional blocking objects for the same task-goal adjacency, generated-per-pair xmls carrying extra boundary objects. Believed benign (room-held-out eval → no leakage; c09 arm shows boards-alone don't drive gains); a verifier agent is producing the exact decomposition + leakage check + distribution comparison — append its ledger here when it reports.
