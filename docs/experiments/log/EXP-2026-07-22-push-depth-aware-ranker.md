...
**Post-verdict default hygiene (user, 2026-07-23).** Action motion remains globally opt-in (`NAMO_ACTION_MOTION=0` by default), because plain relative did not improve mean top-1 ordering. Set `NAMO_ACTION_MOTION=1` to enable plain `crop_relative`; additionally set `NAMO_ACTION_MOTION_SHARP=1` for Fourier features plus learned depth identity. When motion is enabled without naming an encoding, the fallback is the semantically correct and lower-variance `crop_relative` feature rather than the rejected absolute-final-pose feature; explicit legacy/final-pose checkpoint loading remains unchanged. These switches are also documented beside the tunables in the canonical `scripts/slurm/train.slurm` launcher.
