...
- **The "hard setup placement problem" was a LABEL ARTIFACT** (reproduced independently). Against exhaustive truth the model ranks a true setup at hard median rank 2, hit@1 **46.8%** (report) / **50.0%** (orchestrator recompute) vs random 15.6 — the labeled-only view (mean 17.4, hit@1 15.8 ≈ random) understated it ~2.4× because valid_first_push is completion-sampled.
- **Within-board finish ranking is excellent and tier-flat** (reproduced independently): best true finish at median rank 1, hit@1 65–72% (report) / 63–66% (recompute) vs random 12–17%. Not the failure.
- **Cross-board calibration is WEAK — but the agent's "below chance" figure did NOT reproduce.** Orchestrator recompute directly from `testset_gt.h5` + `values.npy` (moved boards only, basename+object tier join): hard true-setup median **0.755** vs dead-board top median **0.655**, root-setup-vs-dead **AUC 0.583** — weak positive discrimination, NOT the reported 0.632/0.675/AUC 0.469 inversion. Live-rate among moved hard boards **28.1%** (dead:live **2.6:1**), vs the report's 19.0% / 6:1. The discrepancy is method-sensitivity (noop-board inclusion + tier-join/episode-set differences); the REPORT.md figures are not independently confirmed and the "dead boards outscore true setups" claim is WITHDRAWN pending reconciliation. What survives: cross-board separation is poor (AUC ~0.58) relative to within-board ranking, so score comparability across boards remains the weakest axis — the flooding driver — but it is not below chance.
> **⚠ RECONCILED 2026-07-26 — [`auc_metrics_reconciliation.md`](../auc_metrics_reconciliation.md).** The `0.583` recompute above does **not** reproduce: the stated construction (setup cell vs moved-dead-board max) gives hard **0.515**, and unrestricted it is REPORT.md's **0.469** — so the "withdrawn" 0.469 was in fact the reproducible figure, and 0.583 is the one to drop. REPORT.md's D2 (0.750/0.768) reproduces exactly. What changes in the reading: this comparison pits ONE setup cell against the MAX of ~70 dead cells, so it is an order statistic, not average mis-scoring — cell-vs-cell the same model separates at **0.892 all / 0.864 hard**. Cross-board comparability is still the weakest axis; the defect is the right tail of the dead-board score distribution.
...
**Adopted at this point in the card: best-first + fitted-g failure discount** (`--discount fitted --gtable round3/eval/gfit/v2/fit/g_table.json`) — **SUPERSEDED the same evening by `--discount conf --tau 0.15`** (see §Patience arms above, which strictly dominates fitted-g). Fitted g remains the calibration reference and the random arm's machinery. Follow-ups queued for the next round: rerun hard1p_h2 + reactive under the new search; re-state parent-card headline numbers in the discount column; timed campaign on the adopted configuration; ~~τ/tempered-conf arm only if the @5 dip matters~~ (RAN same evening — it won; see §Patience arms). Eval outputs `round3/eval/discount_2push/*`; sbatch `run_discount_2push.sbatch`. Plot: `round3/eval/plots/success_vs_sims_discount.png` (anytime curves, all + hard, 5 arms).
