...
**Push-depth implementation (pending target-box gate).** The live 300-action generator now records each board's normalized `(60,5,3)` `action_motion`, current object state, shape family, and primitive-database identifier/SHA256 directly in the collection PKL; the Colossus H5 builder fails on missing/misaligned slots and retains row-level provenance, while the final d20+Colossus mix preserves new-row action fields and marks legacy d20 rows as reconstructable. Colossus also prunes a setup frontier only when the target object moved no more than 1 cm in either axis and no more than 0.05 rad, matching the prior no-op exclusion rule; production remains blocked until a fresh Amarel end-to-end smoke passes.
