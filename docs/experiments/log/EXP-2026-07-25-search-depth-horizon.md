...
> **⚠ CORRECTION 2026-07-26 — the `0.583` anchor is retired; see [`auc_metrics_reconciliation.md`](../auc_metrics_reconciliation.md)** (12-checkpoint × 2-eval-set grid, seed-banded). It does not reproduce: the same setup-cell-vs-moved-dead-board-max construction gives hard **0.515**, unrestricted **0.470** (deploy seed-1; the 07-24 REPORT's 0.469). The *conclusion* survives but the *mechanism* is narrower than "the model can't compare boards": setup cell vs a random dead post-push **cell** is **0.892 all / 0.864 hard** — per cell the model is fine. It only loses to the dead board's **max**, and a dead board offers ~70 draws, so this is a right-tail / order-statistic problem and must be quoted as such. Three further corrections that bear on this card's framing:
> - **Root setup separation is tier-flat** (V1 0.809/0.825/0.814), not hard-degraded — the old slope was a `valid_first_push` label artifact. The tier slope lives in cross-board V5 (0.609/0.562/0.470).
> - **"Within-board strong, cross-board weak" is only half true.** Pooled and within-board root separation are the same (V1 0.827 vs V2 0.820). The gap is specifically root-board vs post-push **dead** boards.
> - **The cross-board metric is not a safe optimisation target on its own.** `colossus_split` has the best V5 (0.624) and the worst finish separation (F1 0.796) of all 12 checkpoints — and the worst hard-1-push deploy tail (@5 64.2). Pushing on cross-board comparability alone would select it.
