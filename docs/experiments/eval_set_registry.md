...
**The silent-truncation bug is now fixed at the source (`cc9fef7`).** `AttemptResult` carries `neighbour_timed_out` and `candidates_unswept` (0 == exhaustive over objects) on both the success and failure paths, copied into `algorithm_stats`. Verified by forcing truncation with a 12 s cap: 7 of 11 episodes recorded `timed_out=True` with the correct unswept count (e.g. `cand=3 unswept=2`), and a complete neighbour correctly reported `unswept=0 timed_out=False`. Those truncated episodes carry `failure_reason=all_pushes_failed` — under the old code that was their ONLY marker, which is exactly why the loss went unnoticed. Any future sweep can be audited with a single field instead of inferring truncation from candidate counts.
...
⚠ Two known portability traps. On 5 problems (40, 277, 373, 463, 504) Amarel's native build and rlab1 settle the scene to a state that differs from the certificate, so the runner refuses them there; arrakis matches. On 4 problems (37, 46, 77, 348) today's room finder sees different door objects than the frozen task; the runner keeps the frozen objects. The 5 refused problems also refuse on the CS EPYC 7352 nodes (rlab3, rlab4, ilab3) and still run on arrakis. Problems 303 and 599 start on every build but fail on every CS machine for every model tried (no-family, the no-family-local-edge model, Random, geometric; on rlab1, arrakis and the EPYC 7352 nodes), while every model seed solves them on Amarel; the CS machines agree with each other exactly on those two. Beyond them, Amarel and CS call counts differ on about 4% of problems per seed in both directions, so compare solve rates and calls from one machine family. First model evaluation: `hy5u-ablations-keyhole600-1mm-v1` in the [model registry](horizon_q_model_registry.md).
