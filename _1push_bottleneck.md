...
**1push = a sampling problem (the opener is in the data; the loss just doesn't see it).** 1push rows carry the exhaustive `f_grid` (all ~58–90 reachable pushes labeled), so the opener IS a labeled positive. But `sample_k=30` uniformly picks 30 reachable cells and computes the loss only on those (deterministic per seed). The unsampled cells (including the opener) are **masked out — no loss term, neither positive nor negative — NOT forced to 0.** That masking is deliberate and correct for *sampled* rows: forcing untried cells to 0 is the measured "C15 poison" (false negatives suppress valid pushes — C15 12.4 < honest-but-fewer B5 14.1 in `policy_framework_journal`), so masking is the right PU treatment. So the miss is **under-training (no signal), not mislabeling (wrong signal)** — the label stays correct, the loss just never touches it. The bug is only that this masking is applied to the *exhaustive* 1push rows, where the label is *known*, needlessly hiding a known positive. For a rare-opener row (~1 positive) the opener lands outside the 30 most of the time — and the starvation rate falls across the rarity slices **in lockstep with the test-time failure**:
