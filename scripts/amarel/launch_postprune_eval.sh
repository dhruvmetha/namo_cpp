...
launch() {
  ...
  job=$(sbatch --parsable --array="0-${last}%${MAX_PARALLEL}" \
    --export="ALL,NAMO_REPO=$REPO,CKPT=$CKPT,KEY=$key,OUT_DIR=$out,HMAX=2,SIM_BUDGET=900,PRIOR=$prior,AGG=mean5,COMBINE=q,DISCOUNT=conf,TAU=0.15,SEED_BASE=$seed,SHARD=$shard" \
    scripts/amarel/bestfirst_eval.slurm)
  printf '%-24s %s\n' "$label" "$job"
}

launch model_1push "$ONEPUSH_KEY" 1323 "$SHARD_1P" model 7000
launch model_2push "$TWOPUSH_KEY" 1018 "$SHARD_2P" model 7000
for seed in 7000 8000 9000; do
  launch "random_s${seed}_1push" "$ONEPUSH_KEY" 1323 "$SHARD_1P" uniform "$seed"
  launch "random_s${seed}_2push" "$TWOPUSH_KEY" 1018 "$SHARD_2P" uniform "$seed"
done
