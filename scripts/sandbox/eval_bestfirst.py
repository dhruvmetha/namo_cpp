...
def candidates(planner, env, goal, xml, state, h, prior, agg, rng):
    """All reachable pushes from `state` with a priority-base value, plus the state value V.
    model: q = Q(state,a,h); V = agg of top Q (mean5 robust, or max). uniform: random q, V=0."""
    pool = rank_first_pushes_h2(planner, env, goal, xml, state, h)   # [(obj, g, q)]
    if not pool:
        return [], 0.0
    if prior == "uniform":
...
def solve_scene(planner, env, goal, xml, s0, hmax, sim_budget, prior, agg, combine, rng):
    """Greedy best-first. Returns (solved, sims_used, plan_len|None)."""
    heap = []; ctr = 0; sims = 0
    pool, V0 = candidates(planner, env, goal, xml, s0, hmax, prior, agg, rng)
    for (obj, g, q) in pool:
    ...
    while heap and sims < sim_budget:
        ...
        if ndone < hmax:                                  # room for another push -> expand the reached state
            ...
            h = hmax - ndone
            pool, V = candidates(planner, env, goal, xml, s_new, h, prior, agg, rng)
...
def main():
    ...
    print(f"device={planner.scorer.device} hmax={a.hmax} sim_budget={a.sim_budget} prior={a.prior} "
          f"agg={a.agg} combine={a.combine}", flush=True)
    xmls = read_manifest(a.manifest, None)[a.start:a.end]
    n = n_solved = n_already = sims_tot = sims_solved = 0; t0 = time.time()
    lf = open(a.leaf_out, "w")
    for xi, xml in enumerate(xmls):
        try:
            rng = random.Random(7000 + xi)
            env = make_env(xml)
            ...
            solved, sims, plen = solve_scene(planner, env, goal, xml, s0, a.hmax, a.sim_budget,
                                             a.prior, a.agg, a.combine, rng)
            n += 1; sims_tot += sims; n_solved += int(solved); sims_solved += sims if solved else 0
            lf.write(json.dumps({"xml": xml, "solved": solved, "sims": sims, "plan_len": plen}) + "\n")
            if xi % 20 == 0:
                print(f"  [{xi}/{len(xmls)}] n={n} solved={n_solved} avg_sims={sims_tot/max(n,1):.1f} "
    ...
    res = {"ckpt": a.ckpt, "hmax": a.hmax, "sim_budget": a.sim_budget, "prior": a.prior, "agg": a.agg,
           "combine": a.combine, "n": n, "n_already_open": n_already,
