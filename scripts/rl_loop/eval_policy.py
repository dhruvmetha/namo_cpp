#!/usr/bin/env python3
"""Policy rollout: rank, execute, observe, up to --max-pushes (fixed region and objects).
...
from namo.planners.opening.best_first_search import _unmoved, solve_scene  # noqa: E402
...
def main():
    ...
    ap.add_argument("--groups", help="Frozen group pilot JSONL from probe_static_topology")
    ap.add_argument("--mode", choices=["policy", "search"], default="policy")
    ap.add_argument("--hmax", type=int, default=2)
    ap.add_argument("--sim-budget", type=int, default=900)
    ...
    key = json.load(open(a.key)) if not a.groups else {}
    if a.groups:
        for line in open(a.groups):
            group = json.loads(line)
            if not group["eligible"]:
                raise ValueError("Group pilot contains an ineligible group")
            key.setdefault(group["room_realpath"], []).append({
                "object_id": group["group_id"], "objects": group["boundary_objects"],
                "region": group["goal_label"], "group": group,
            })
    ...
    for xi, xml in enumerate(xmls):
        for rec in key[xml]:
            ...
            try:
                ...
                gp = rec["group"]["target_points"] if a.groups else sample_goal_points(env)
            except Exception as exc:
                raise RuntimeError(f"Failed episode initialization: {xml}/{obj}/{reg}") from exc
            if not gp or goal_open_pts(env, gp):
                if a.groups:
                    raise RuntimeError(f"Frozen group is empty or already open: {obj}")
                skip += 1; continue
            ...
            if a.mode == "search":
                import numpy as np
                solution = {}
                solved, sims, length, _, end = solve_scene(
                    pl, env, goal, xml, s0, a.hmax, a.sim_budget,
                    "model" if a.prior == "q" else "uniform", "mean5", "q",
                    np.random.default_rng(a.seed), restrict_obj=allowed,
                    is_open=lambda e: goal_open_pts(e, gp), raw=True,
                    discount="off", dedupe_noop=True, prune_jam_depth=True,
                    region_samples=gp if a.groups else None, solution_out=solution,
                )
                leaf.append({"xml": xml, "object_id": obj, "region": reg,
                             "group": rec.get("group"), "solved": solved, "sims": sims,
                             "plan_len": length, "end": end, "mode": "search",
                             "hmax": a.hmax, "sim_budget": a.sim_budget,
                             "actions": [{"object_id": o, "edge_idx": int(g.edge_idx),
                                          "depth": int(g.depth)} for o, g in solution.get("plan", [])]})
                n += 1
                continue
            pool = rank_first_pushes_h2(pl, env, goal, xml, s0, a.h, restrict_obj=allowed,
                                       score=(a.prior == "q"), raw=bool(a.groups),
                                       region_samples=gp if a.groups else None)
            if not pool:
                if a.groups:
                    raise RuntimeError(f"Frozen group has no initial candidate: {obj}")
                skip += 1; continue
            n += 1
            opened_at = 0
            ...
            for pidx in range(1, K + 1):
                if pidx > 1:  # re-rank at the LIVE state; finishes query H=1
                    pool = rank_first_pushes_h2(pl, env, goal, xml, s_cur, 1, restrict_obj=allowed,
                                               score=(a.prior == "q"), raw=bool(a.groups),
                                               region_samples=gp if a.groups else None)
                live = [(o, g, q) for (o, g, q) in pool
                        if (o, int(g.edge_idx), int(g.depth)) not in banned
                        and not (a.jam_prune and jam_at.get((o, int(g.edge_idx))) is not None
                                 and int(g.depth) >= jam_at[o, int(g.edge_idx)])]
                if not live:
                ...
                res = env.step(make_action(_o, g)); n_push += 1
                actions.append({"object_id": _o, "edge_idx": int(g.edge_idx), "depth": int(g.depth)})
                ...
                if a.jam_prune and (res.info or {}).get("failure_reason"):
                    jd = jam_at.get((_o, edge))
                    if jd is None or depth < jd:
                        jam_at[_o, edge] = depth
                if a.dedupe_noop and _unmoved(obs_before, env.get_observation(), _o):
                    n_noop += 1
                    banned.add((_o, edge, depth))
            ...
            r_leaf = {"xml": xml, "object_id": obj, "region": reg, "opened_at": opened_at,
                      ...
                      "actions": actions, "group": rec.get("group"), "mode": "policy"}
