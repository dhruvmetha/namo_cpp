...
for arm in "${arms[@]}"; do
  ...
  smoke_job=$(sbatch --parsable \
    ...
    --export="ALL,NAMO_REPO=$REPO,SAGE_REPO=$SAGE_REPO,NAMO_BINDINGS=$NAMO_BINDINGS,CKPT_ROOT=$CKPT_ROOT,CKPT=$CKPT_ROOT/${arm}_s1.ckpt,OUT=$smoke_out,N1SH=7,N2SH=7,WORKERS_PER_TASK=14,ONEPUSH_LIMIT=7,TWOPUSH_LIMIT=7,REFUSE_OVERWRITE=1,HMAX=2,SIM_BUDGET=900,PRIOR=model,SEED_BASE=7000" \
  ...
  for seed in 1 2 3; do
    ...
    full_job=$(sbatch --parsable --dependency="afterok:$smoke_job" --kill-on-invalid-dep=yes \
      ...
      --export="ALL,NAMO_REPO=$REPO,SAGE_REPO=$SAGE_REPO,NAMO_BINDINGS=$NAMO_BINDINGS,CKPT=$CKPT_ROOT/${arm}_s${seed}.ckpt,OUT=$eval_out,N1SH=378,N2SH=378,WORKERS_PER_TASK=21,REFUSE_OVERWRITE=1,HMAX=2,SIM_BUDGET=900,PRIOR=model,SEED_BASE=7000" \
      scripts/slurm/aquaman_eval_amarel.slurm)
    ...
    agg_job=$(sbatch --parsable --dependency="afterok:$full_job" --kill-on-invalid-dep=yes \
      ...
      --export="ALL,NAMO_REPO=$REPO,SAGE_REPO=$SAGE_REPO,NAMO_BINDINGS=$NAMO_BINDINGS,EVAL_ROOT=$eval_out" \
