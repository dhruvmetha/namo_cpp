#!/bin/bash
# v4 endgame: wait on SHARD COUNTS (not log markers), pull, aggregate, build the comparison.
#
...
#
# Expected counts differ per arm: the AJ4 trio was submitted at 72 shards, the AJ4NR trio at 144
...
REM=${AMAREL_ROOT}/aquaman0/eval_v4
MAIN=/common/home/dm1487/robotics_research/ktamp/namo
PY=/common/users/dm1487/envs/mjxrl/bin/python

want() { case "$1" in AJ4_s*) echo 72;; *) echo 144;; esac; }

for i in $(seq 1 480); do          # up to 8 h
  ok=1
  for m in AJ4_s1 AJ4_s2 AJ4_s3 AJ4NR_s1 AJ4NR_s2 AJ4NR_s3; do
...
[ "$ok" -eq 1 ] || { echo "TIMED OUT waiting for v4 shards"; exit 1; }
echo "ALL V4 SHARDS COMPLETE $(date)"

rsync -a --include='*/' --include='shard_*.jsonl' --include='shard_*.json' --exclude='*' \
  "amarel:$REM/" "$R0/eval_v4/"
echo "pulled $(ls $R0/eval_v4/*/{1push_hmax2,2push}/shard_*.jsonl 2>/dev/null | wc -l) jsonl"

cat > "$R0/arms_aj4_full.json" <<EOF
{"AJ4":   {"1push": ["$R0/eval_v4/AJ4_s1/1push_hmax2","$R0/eval_v4/AJ4_s2/1push_hmax2","$R0/eval_v4/AJ4_s3/1push_hmax2"],
           "2push": ["$R0/eval_v4/AJ4_s1/2push","$R0/eval_v4/AJ4_s2/2push","$R0/eval_v4/AJ4_s3/2push"]},
 "AJ4NR": {"1push": ["$R0/eval_v4/AJ4NR_s1/1push_hmax2","$R0/eval_v4/AJ4NR_s2/1push_hmax2","$R0/eval_v4/AJ4NR_s3/1push_hmax2"],
           "2push": ["$R0/eval_v4/AJ4NR_s1/2push","$R0/eval_v4/AJ4NR_s2/2push","$R0/eval_v4/AJ4NR_s3/2push"]}}
EOF
"$PY" "$MAIN/scripts/rl_loop/aquaman_agg.py" "$R0/arms_aj4_full.json" "$R0/gate_aj4.json"
"$PY" "$R0/build_final_comparison.py" > "$R0/final_comparison.out" 2>&1
echo "V4 COMPLETE AND AGGREGATED $(date)"
