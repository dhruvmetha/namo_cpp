...
Five properties, each with the failure it catches:
  ...
  an unknown mode refuses         "reactive", "argmax", "polciy" all become the
                                  search arm without a word
  policy needs the ranker pool    region_bfs sweeps every edge and depth and
...
def test_the_vocabulary_is_closed_and_defaults_to_search():
    ...
    assert set(BOUNDARY_MODES) == {MODE_SEARCH, MODE_POLICY}
...
@pytest.mark.parametrize("mode", ["reactive", "argmax", "polciy", "", "Search"])
...
def test_policy_refuses_the_planner_that_has_no_ranked_pool():
    ...
    with pytest.raises(ValueError) as excinfo:
        _solve(mode=MODE_POLICY, local_search="region_bfs")

    message = str(excinfo.value)
    assert "best_first" in message, f"the refusal must name the way out: {message}"


@pytest.mark.parametrize("mode", [MODE_SEARCH, MODE_POLICY])
...
def test_the_planner_refuses_an_unknown_decision_rule():
    ...
    with pytest.raises(ValueError, match="decision_rule"):
        _best_first(decision_rule="reactive")
