...
from viz.build_gt_json import build_all, build_episode_gt, green_sets_from_grid  # noqa: E402
...
def test_build_all_agrees_with_build_episode_gt(synthetic_h5, tmp_path):
    key = {"/x/a.xml": [{"object_id": "obj1"}], "/x/missing.xml": [{"object_id": "obj9"}]}
    out_dir = tmp_path / "out"
    out_dir.mkdir()
    with h5py.File(synthetic_h5, "r") as f:
        covered, uncovered = build_all(f, key, str(out_dir))
        expected = build_episode_gt(f, "/x/a.xml", "obj1")

    assert covered == 1
    assert uncovered == [["/x/missing.xml", "obj9"]]

    written = list(out_dir.iterdir())
    assert len(written) == 1
    assert written[0].name == episode_filename("/x/a.xml", "obj1")
    assert json.load(open(written[0])) == expected
